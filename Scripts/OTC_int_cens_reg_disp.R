...
mn=mean(disp$midpoint, na.rm=T)
sd=sd(disp$midpoint, na.rm=T)
...
df=dispcheck
dispcheck$treatment<-factor(dispcheck$treatment)
...
dispcheck2<-filter(dispcheck, all %in% regmodsy$all)%>%select(all, doy, prior_visit)%>%distinct(.)
regmodsy<-left_join(regmodsy, dispcheck2, by='all')%>%mutate(., estimate=(doy+prior_visit)/2)

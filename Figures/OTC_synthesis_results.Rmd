...
#scale added here is the std dev of the OTC-CTL estimates for each phenophase to unscale the mean-centered doy data (see OYC_int_cens_reg scripts)
...
est<-dplyr::select(post2, treatmentOTC,phen)%>%group_by(phen)%>%summarise(Estimate=mean(treatmentOTC))
...
geom_vline(data = ddply(post3, "facet"), aes(xintercept=CI_low_90*scale),color="red",lty=2,size=1)+
geom_vline(data = ddply(post3, "facet"), aes(xintercept=CI_high_90*scale), color="red",lty=2,size=1)+
geom_vline(data = ddply(post3, "facet"), aes(xintercept=CI_low_95*scale),color="grey",lty=2,size=1)+
geom_vline(data = ddply(post3, "facet"), aes(xintercept=CI_high_95*scale),color="grey",lty=2,size=1)+
...
postm6xdisp$N<-postm6xdisp$b_treatmentOTC
postm6xdisp$Y<-postm6xdisp$`b_treatmentOTC:OTCWinterRemovalY`+postm6xdisp$N
postm6xdisp<-mutate(postm6xdisp, diff=Y-N, Estimate=mean(diff), scale=21.35, phen="disp")
...
postm3xgreen<-mutate(postm3xgreen, scale=11.85, phen="green")

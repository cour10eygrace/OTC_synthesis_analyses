...
postflow2<- as_tibble(fixef(fit_m2x_flow, summary=F))%>%dplyr::select(-Intercept)%>%mutate(phen="Flower", scale=15.82, order=5)
postflowend2<- as_tibble(fixef(fit_m2x_flowend, summary=F))%>%dplyr::select(-Intercept)%>%mutate(phen="Flowerend", scale=15.75, order=4)
postfruit2<- as_tibble(fixef(fit_m2x_fruit, summary=F))%>%dplyr::select(-Intercept)%>%mutate(phen="Fruit", scale=16.21, order=3)
postdisp2<- as_tibble(fixef(fit_m2x_disp, summary=F))%>%dplyr::select(-Intercept)%>%mutate(phen="Disp",scale=21.35, order=2)
postgreen2<- as_tibble(fixef(fit_m2x_green, summary=F))%>%dplyr::select(-Intercept)%>%mutate(phen="Green", scale=11.85, order=6)
postsen2<- as_tibble(fixef(fit_m2x_sen, summary=F))%>%dplyr::select(-Intercept)%>%mutate(phen="Sen", scale=12.88, order=1)

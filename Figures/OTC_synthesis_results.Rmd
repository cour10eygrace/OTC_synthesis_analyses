...
postm6xdisp<-select(postm6xdisp, phen, scale, Estimate, Err)%>%distinct(.)
...
postm5xflow<-select(postm5xflow, phen, scale, Estimate, Err)%>%distinct(.)
...
postm7xdisp<-as_tibble(posterior_samples(fit_m7x_disp, pars = 
  c("b_treatmentOTC:siteT")))%>%mutate(scale=dispscales[1,2], phen="Disp")
...
postgreensen<-(postsen2$treatmentOTC*senscales$sd)-(postgreen2$treatmentOTC*greenscales[1,2])
postflowflowend<-(postflowend2$treatmentOTC*flowendscales$sd)-(postflow2$treatmentOTC*flowscales$sd)

```
*calculate length (days)  between greenup and senescence  
```{r Growing season length, echo=TRUE,  warning=FALSE, error=FALSE, message=F}

mean(postgreensen)
sd(postgreensen)
ci(postgreensen, method = "ETI", ci = c(0.9, 0.95))

```

*calculate length (days) between flowering and end of flowering 
```{r Flowering period length, echo=TRUE,  warning=FALSE, error=FALSE, message=F}
mean(postflowflowend)
sd(postflowflowend)
ci(postflowflowend, method = "ETI", ci = c(0.9, 0.95))
